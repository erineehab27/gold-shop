...
export default function App() {
  ...
  const addInvoice = (e) => {
    ...
    // 🔥 السداد
    const paid21 = Number(invoiceForm.paid21 || 0);

    const paid24 = Number(invoiceForm.paid24 || 0);
    const paid24To21 = getEquivalent21(paid24, "24");

    const cash = Number(invoiceForm.cash || 0);
    const goldPrice21 = Number(invoiceForm.goldPrice21 || 1);
    const cashTo21 = cash / goldPrice21;

    const bondoqi = Number(invoiceForm.bondoqi || 0);
    const bondoqiTo21 = bondoqi * (991 / 875);

    const sabekaWeight = Number(invoiceForm.sabekaWeight || 0);
    const sabekaKarat = Number(invoiceForm.sabekaKarat || 0);
    const sabekaTo21 = sabekaWeight * (sabekaKarat / 875);

    const totalPaid21 =
      paid21 + paid24To21 + cashTo21 + bondoqiTo21 + sabekaTo21;

    const remaining21 = equivalent21 - totalPaid21;

    const remainingMoney =
      total - (totalPaid21 * gramPrice);
  ...
  const total21Weight = customerInvoices.reduce(
    (acc, inv) => acc + (inv.equivalent21 || 0),
    0
  );

  const balance = customerInvoices.reduce((acc, inv) => {
    if (inv.type === "sale") return acc + inv.remainingMoney;
    else return acc - inv.remainingMoney;
  }, 0);
      ...
      <div className="grid">
        ...
        <section className="card">
          ...
          <form onSubmit={addInvoice}>
            ...
            <input placeholder="عيار 21" onChange={(e)=>setInvoiceForm({...invoiceForm, paid21:e.target.value})}/>
            <input placeholder="عيار 24" onChange={(e)=>setInvoiceForm({...invoiceForm, paid24:e.target.value})}/>
            <input placeholder="نقدي" onChange={(e)=>setInvoiceForm({...invoiceForm, cash:e.target.value})}/>
            <input placeholder="سعر 21 للنقدي" onChange={(e)=>setInvoiceForm({...invoiceForm, goldPrice21:e.target.value})}/>
            <input placeholder="بندقي" onChange={(e)=>setInvoiceForm({...invoiceForm, bondoqi:e.target.value})}/>
            <input placeholder="سبيكة وزن" onChange={(e)=>setInvoiceForm({...invoiceForm, sabekaWeight:e.target.value})}/>
            <input placeholder="سبيكة عيار" onChange={(e)=>setInvoiceForm({...invoiceForm, sabekaKarat:e.target.value})}/>
      ...
      <section className="card">
        ...
        {selectedCustomer && (
          <>
            ...
            <h3>إجمالي 21: {total21Weight.toFixed(2)} جرام</h3>
              ...
              <tbody>
                {customerInvoices.map((inv) => (
                  <tr key={inv.id}>
                    <td>{inv.id}</td>
                    <td>{inv.equivalent21?.toFixed(2)}</td>
                    <td>{inv.totalPaid21?.toFixed(2)}</td>
                    <td>{inv.remaining21?.toFixed(2)}</td>
                    <td>{inv.remainingMoney?.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
