...
export default function StockPage() {
  ...
  const [customersWorkDebt, setCustomersWorkDebt] = useState(0);
...
invoices.forEach((inv) => {
  totalWork += Number(inv.remainingWork || 0);
});

setCustomersWorkDebt(totalWork);
  ...
  return (
    <div className="app">
      ...
      <section className="card">
        ...
        <p>🟡عندي دهب عيار 21: {fix(gold21)}</p>
        <p>📈 ليا عند العملاء (ذهب 21): {fix(customersDebt)}</p>
<p>🛠️ ليا مصنعية عند العملاء: {fix(customersWorkDebt)}</p>
        <p>🏭 عليا للموردين (ذهب عيار 21): {fix(suppliersBalance)}</p>
<p>🛠️ عليا مصنعية للموردين: {fix(suppliersWorkBalance)}</p>

        <hr />

        <h2>📊الصافي الحقيقي (عيار21):{net}</h2>
