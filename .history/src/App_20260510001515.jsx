...
export default function App() {
  ...
  // 🔥 التحويل الصح
  const to21 = (w, k) => {
    if (k === 24) return fix((w * 999.9) / 875);
    if (k === 21) return fix(w);
    return fix((w * k) / 875);
  };
  ...
  // سداد مباشر
  totalWeight21 -= Number(payGold || 0);
  totalWorkmanship -= Number(payWork || 0);

  const remainingGold = fix(totalWeight21);
  const remainingWork = fix(totalWorkmanship);
    ...
    <div className="app">
      ...
      <div className="grid">
        ...
        <section className="card">
          ...
          <h3>سداد</h3>
          <input placeholder="سداد ذهب 21" onChange={(e) => setPayGold(e.target.value)} />
          <input placeholder="سداد مصنعية" onChange={(e) => setPayWork(e.target.value)} />
