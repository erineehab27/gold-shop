...
export default function PrintPage() {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      {(oldGold || oldWorkmanship) && (
        <>
          <h3>حساب قديم</h3>
          {oldGold && <p>وزن 21: {oldGold}</p>}
          {oldWorkmanship && <p>مصنعية: {oldWorkmanship}</p>}
          <hr />
        </>
      )}

      {(payGold || payGold24 || payWorkmanshipNow || cashBack || barWeight) && (
        <>
          <h3>سداد جديد</h3>
          ...
          {barWeight && (
            <>
              <p>وزن سبيكة: {barWeight}</p>
              <p>عيار سبيكة: {barKarat}</p>
