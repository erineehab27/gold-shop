...
export default function App() {
  ...
  // تحويل 24 → 21
  const convert24to21 = (w) => w * (999.9 / 875);
  ...
  const addInvoice = (e) => {
    ...
    const paidGold = Number(invoiceForm.paidGold || 0);
    const paidCash = Number(invoiceForm.paidCash || 0);

    const remainingGold = totalWeight21 - paidGold;
    const remainingCash = totalWorkmanship - paidCash;
  ...
  return (
    <div className="app">
      ...
      {/* الفاتورة */}
      <form onSubmit={addInvoice} className="card">
        <h2>فاتورة</h2>
        ...
        <h3>السداد</h3>
        <input
          placeholder="مدفوع دهب (يتحسب 21)"
          value={invoiceForm.paidGold}
          onChange={(e) =>
            setInvoiceForm({ ...invoiceForm, paidGold: e.target.value })
          }
        />

        <input
          placeholder="مدفوع فلوس"
          value={invoiceForm.paidCash}
          onChange={(e) =>
            setInvoiceForm({ ...invoiceForm, paidCash: e.target.value })
          }
        />
