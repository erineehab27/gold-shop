...
export default function App() {
  ...
  const addInvoice = (e) => {
    ...
    const workmanship = Number(invoiceForm.workmanship || 0);
    const discount = Number(invoiceForm.discount || 0);

    const paidWeight = Number(invoiceForm.paid_weight || 0);
    const paidMoney = Number(invoiceForm.paid_money || 0);

    // نحسب سعر الجرام التقريبي
    const gramPrice =
      equivalent21 > 0 ? (workmanship - discount) / equivalent21 : 0;

    const remainingWeight = equivalent21 - paidWeight;
    const remainingMoney =
      remainingWeight * gramPrice - paidMoney;
  ...
  const totalRemainingWeight = customerInvoices.reduce(
    (acc, inv) => acc + (inv.remainingWeight || 0),
    0
  );

  const totalRemainingMoney = customerInvoices.reduce(
    (acc, inv) => acc + (inv.remainingMoney || 0),
    0
  );
  ...
  return (
    <div className="app">
      ...
      <div className="grid">
        ...
        <section className="card">
          ...
          <form onSubmit={addInvoice}>
            ...
            <input
              placeholder="مدفوع جرام"
              value={invoiceForm.paid_weight}
              onChange={(e) =>
                setInvoiceForm({
                  ...invoiceForm,
                  paid_weight: e.target.value,
                })
              }
            />

            <input
              placeholder="مدفوع فلوس"
              value={invoiceForm.paid_money}
              onChange={(e) =>
                setInvoiceForm({
                  ...invoiceForm,
                  paid_money: e.target.value,
                })
              }
            />
      ...
      <section className="card">
        ...
        {selectedCustomer && (
          <>
            <h3>إجمالي عيار 21: {total21Weight.toFixed(2)} جرام</h3>
            <h3>المتبقي: {totalRemainingWeight.toFixed(2)} جرام</h3>
            <h3>المتبقي فلوس: {totalRemainingMoney.toFixed(2)} جنيه</h3>
              ...
              <tbody>
                {customerInvoices.map((inv) => (
                  <tr key={inv.id}>
                    <td>{inv.weight}</td>
                    <td>{inv.equivalent21?.toFixed(2)}</td>
                    <td>{inv.remainingWeight?.toFixed(2)}</td>
