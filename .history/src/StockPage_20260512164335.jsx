...
export default function StockPage() {
  ...
  const [customersDebt, setCustomersDebt] = useState(0);
  const [suppliersDebt, setSuppliersDebt] = useState(0); // عليكي
  const [suppliersCredit, setSuppliersCredit] = useState(0); // ليكي
  ...
  useEffect(() => {
    ...
    // 🏭 الموردين
    let totalDebt = 0;   // عليكي
    let totalCredit = 0; // ليكي
    ...
    supplierRecords.forEach((r) => {
      ...
      let remaining = Number(r.totalGold21 || 0) - Number(r.payGold21 || 0);

      if (remaining > 0) {
        totalDebt += remaining; // عليكي
      } else {
        totalCredit += Math.abs(remaining); // ليكي
      }
    });
    ...
    invoices.forEach((inv) => {
      ...
      // 🔥 تجميع الأصناف في الخزنة
      if (inv.items) {
        setItems(prev => [...prev, ...inv.items]);
      }
    });
  ...
  const net = fix(
    ...
    cash +
    customersDebt +
    suppliersCredit - // ليكي من الموردين
    suppliersDebt -   // عليكي للموردين
    0
  );
